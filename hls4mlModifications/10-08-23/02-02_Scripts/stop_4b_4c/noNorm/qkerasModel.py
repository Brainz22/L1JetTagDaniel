...
def main(args):
    ...
    x = QActivation(activation=quantized_relu(10, 4) , name="q_activation")(x)
    ...
    x = QActivation(activation=quantized_relu(10, 4), name="q_activation_1")(x)
    ...
    x = QActivation(activation=quantized_relu(10, 4,), name="q_activation_2")(x)
    x = QDense(
        1,
        kernel_quantizer=quantized_bits(10, 4, alpha=1),
        bias_quantizer=quantized_bits(10, 4, alpha=1),
    ...
    plt.xlabel("Weights")
    plt.savefig("{}/{}_weights.png".format(args.llpType, tag))

    np.save("{}/{}_qkmodelWeights.npy".format(args.llpType,tag), weights)
    np.save("{}/{}_ptRange.npy".format(args.llpType, tag), sampleData[:,0])
    ...
    plt.tight_layout()
    plt.savefig(str(args.llpType) + "/{}_qkLoss.pdf".format(tag) , dpi=120 )
    model = tfmot.sparsity.keras.strip_pruning(model)
    # Save the network
    #model.save(str(args.llpType) + "/"+ str(args.llpType)+ "qkL1JetTagModel.h5")
    model.save(str(args.llpType) + "/{}_qkL1JetTagModel.h5".format(tag))
