...
def main(args):
    ...
    for entryNum in pbar:
        ...
        for i in range(len(obj)):
            ...
            if i not in bannedParts:  # Identifies highest avaiable pT particle to use as seed
                    ...
                    if (
                        ...
                        if tree.gen[e][0].DeltaR(tempTLV) <= DELTA_R_MATCH: #Check that LLP is within jet (DeltaR)
                            ...
                            if pc > 0: #Only add signal flag and ban LLP if a product is also inside jet
                                ...
                                assert len(LLPinfo) == 0, "\n 1) LLPinfo length {} wrong when adding LLP information at event {} \n ".format(len(LLPinfo), entryNum)
                            ...
                            elif (pc == 0 and len(LLPinfo)==0):

                                assert pc == 0, "\n pc not equal 0. \n"
                                assert len(LLPinfo) == 0, "\n Wrong length LLPinfo: LLPinfo {}, event {}".format(LLPinfo, entryNum)
                ...
                if len(LLPinfo) == 0: 
                    LLPinfo.extend((0, 0, 0, 0))
                assert len(LLPinfo) == 4, "\n 3) LLP info has length {} at event {}\n".format(len(LLPinfo), entryNum)
                jetPartList.extend(LLPinfo)

                jetNum += 1

                #assert len(jetPartList) == len(jetPartsArray), "No matching lengths. JetPart list {} vs. jetPartsArray {}".format(len(jetPartList), len(jetPartsArray) )
    ...
    assert len(trainArray) == len(trainingFullData), "No matching lengths. trainArray {} vs. trainingFullData {}".format(len(trainArray), len(trainingFullData) )
    ...
    assert len(testArray) == len(jetFullData) and len(trainArray) == len(trainingFullData), "ISSUE HERE. No matches"
